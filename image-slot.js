...
(() => {
  ...
  function loadHeic2any() {
    ...
    if (heic2anyP) return heic2anyP;
    heic2anyP = new Promise((resolve, reject) => {
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      // Floor the target width so a slot that hasn't finished laying out yet
      // (0 or very small clientWidth at drop time) doesn't get resampled down
      // to a tiny, permanently-blurry cache.
      const safeW = Math.max(targetW || 0, 480);
      const cap = Math.min(MAX_DIM, Math.max(1, Math.round(safeW * 2)) || MAX_DIM);
  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      ...
      if (!file || (ACCEPT.indexOf(file.type) < 0 && !isHeic)) {
        this._setError('Drop a PNG, JPEG, WebP, AVIF, or HEIC image.');
        return;
      }
      ...
      const gen = ++this._gen;
      try {
        // Most non-Safari browsers can't decode HEIC/HEIF natively (createImageBitmap
        // would just fail), so convert to JPEG client-side first via heic2any.
        if (isHeic) {
          this._setError('Conversion HEIC en cours…');
          try {
            file = await heicToJpeg(file);
          } catch (heicErr) {
            if (gen !== this._gen) return;
            console.warn('<image-slot> HEIC conversion failed:', heicErr);
            this._setError('Échec de la conversion HEIC — réessaie, ou exporte la photo en JPEG depuis ton téléphone puis redépose-la.');
            return;
          }
          if (gen !== this._gen) return;
          this._setError(null);
